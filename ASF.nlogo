...
to setup
  clear-all
  create-turtles number_of_person
  [
  ...
  ask one-of turtles with [human? = false] [set virus? true]
  ask turtles [ recolor ]
  reset-ticks
end
...
to move ;; Turtles movement with classification if that turtle has partner or not
  let entities turtles with [partner = nobody]
...
to handshake ;; Interact with turtles partner
  ifelse dead? []
  [
    if (partner = nobody) and (any? other turtles-here with [partner = nobody] and human? = false)
    [
...
to leave ;; Leave the partner in a certain random time
  if interaction? = true
  [
...
to recolor
  ifelse virus? [set color red set time (20 + random 50) ] [set color blue]
  if dead? = true [ set color grey ]
  ifelse interaction?
  [
    if human? = false
    [
